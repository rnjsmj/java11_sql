...
insert into sales values(sseq.nextval, 1, 'kkt', 2, ((select bookprice from book where book.bookid = 1) * 2), default);
u
...
alter table book2 add ckdata varchar(60);
...
alter table member2 add constraint mem_key primary key (id);
select * from ALL_CONSTRAINTS where OWNER='C##SQLTEST';
...
--**** 외래키
alter table book2 add constraint book2_fk foreign key (bookkind) references bookkind(kindcode);
--****

-- 	24) 도서2(book2) 테이블에서 출판일(pubdate)가 2022년 8월 인 데이터의 수량을 5씩 더 증가시키시오.																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																														
update book2 set bookcount = bookcount+5 where pubdate like '22/08/%';
...
select distinct id from member where id in (select id from sales);
...
select * from book where pubdate like '22/%';
...
-- 35) 도서(book) 테이블에서 도서의 수량(bookcount)가 10권 미만인 튜플에 대하여 도서분류(bookkind), 도서명(booktitle), 
-- 출판사(pubcom) 을 검색하되 그 결과가 저자(author)의 오름차순으로 정렬하여 표시되도록 하시오.
select bookkind, booktitle, pubcom from book where bookcount<10 order by author;

-- 36) 도서(book) 테이블에서 도서분류(bookkind)가 IT, NV, TC가 아닌 레코드의 도서코드(bookid), 도서명(booktitle), 
--저자(author) 를 검색하되 그 결과가 출판일을 기준으로 내림차순되어 표시되도록 하시오.
...
select to_char(reg_date, 'YY/MM/DD') as reg_date, count(*) from member group by to_char(reg_date, 'YY/MM/DD') order by reg_date;
select reg_date, count(*) as "인원수" from member group by reg_date order by reg_date;

-- 41) 도서(book) 테이블에서 도서수량(bookcount)가 남은 수량이 적은 것을 기준으로 5위권까지 
-- 모든 도서 정보가 출력되도록 하시오.
...
-- 44) 판매 뷰(sales_view)를 생성하되 판매2(sales2) 테이블을 활용하고, 판매코드(sno), 도서코드(bno), 아이디(id), 
--판매금액(money) 컬럼만 추출되어 생성되게 하시오.
create table sales2 as select * from sales;
create view sales_view as select sno, bno, id, money from sales2;
select * from sales_view;

-- 45) 도서 뷰(book_view)를 생성하되 도서2(book2) 테이블을 활용하고, 도서 데이터 중에서 도서분류(bookkind)가 
-- 'IT', 'TC', 'HC' 인 데이터를 대상으로 하며, 컬럼은 도서분류(bookkind), 도서명(booktitle), 
-- 도서가격(bookprice), 출판사(pubcom) 만으로 구성되게 하시오.
...
create or replace view book_view as select bookkind, booktitle, bookprice * 1.1 as bookprice, pubcom from book where bookkind in ('IT', 'TC', 'HC');
select * from book;

-- 47) 판매 뷰(sales_view)를 편집하되 기존 select 구문에서 수량(amount) 가 2이상인 조건을 추가되게 하시오.
create or replace view sales_view as select sno, bno, id, money from sales2 where amount >= 2;
select * from sales_view;

-- 48) 회원 뷰(mem_view)에서 아이디(id)가 y로 끝나는 회원의 데이터를 삭제하시오.
create or replace view mem_view as select * from member2 where id not like '%y';
select * from mem_view;
...
-- 58) 내부조인을 활용하여 판매된 적이 있는 도서이름(booktitle), 도서가격(bookprice), 판매수량(amount), 
-- 판매금액(money) 을 조회하시오.
select b.booktitle, b.bookprice, sum(s.amount) as amount, sum(s.money) as money 
from book b inner join sales s on b.bookid = s.bno group by b.booktitle, b.bookprice;
...
select m.id, m.name, sum(s.amount) as amount, sum(s.money) as money
from member m inner join sales s on m.id = s.id group by m.id, m.name;

select m.id, m.name, s.amount, s.money from member m inner join sales s on m.id = s.id;

-- 59) 외부조인을 활용하여 판매되지 않은 도서의 도서명(booktitle), 도서가격(bookprice), 저자(author) 을 조회하시오.
select distinct b.booktitle, b.bookprice, b.author
from book b left outer join sales s on b.bookid = s.bno
minus
select distinct b.booktitle, b.bookprice, b.author
from book b right outer join sales s on b.bookid = s.bno;

-- 60) 외부조인을 활용하여 구매한 적이 없는 회원의 회원아이디(id), 회원명(name) 을 조회하시오.
select distinct m.id, m.name from
member m left outer join sales s on m.id = s.id
minus
select distinct m.id, m.name from
member m right outer join sales s on m.id = s.id;
...
select * from book;
select * from book2;
alter table book add ckdata varchar(60);